...
void OpenGLDisplay::DisplayVideoFrame(unsigned char **yuv, uint16_t *line_size, int frameWidth, int frameHeight)
{
    
    impl->mVideoW = frameWidth;
    impl->mVideoH = frameHeight;
    int frameBytes = impl->mVideoW * impl->mVideoH;
    unsigned char* dst_yuv = reinterpret_cast<unsigned char*>(impl->mBufYuv);
    unsigned char* dst_y = dst_yuv; int src_y_line_size = line_size[0];
...
//    memcpy(dst_yuv + frameBytes, yuv[1], frameBytes / 4);
//    memcpy(dst_yuv + frameBytes * 5/4, yuv[2], frameBytes / 4);
    update();
}
