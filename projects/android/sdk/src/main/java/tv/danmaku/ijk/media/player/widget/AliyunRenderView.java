...
public class AliyunRenderView extends AbstractPlayerView {
    ...
    private void initPlayerListener() {
        ...
        mAliPlayer.setOnReportEventListener(new OnAVPReportEventListener(this));
    }
    ...
    public void setMaxAccurateSeekDelta(int var1) {
        if (mAliPlayer != null) {
            mAliPlayer.setMaxAccurateSeekDelta(var1);
        }
    }
     ...
     */
    private static class OnAVPReportEventListener implements IPlayer.OnReportEventListener {

        private WeakReference<AliyunRenderView> weakReference;

        private OnAVPReportEventListener(AliyunRenderView aliyunRenderView) {
            weakReference = new WeakReference<>(aliyunRenderView);
        }
        ...
        public void onEventParam(Map<String, String> map) {
            AliyunRenderView aliyunRenderView = weakReference.get();
            if (aliyunRenderView != null) {
                aliyunRenderView.onReportEvent(map);
            }
        }
    }

    private IPlayer.OnReportEventListener mOnReportEventListener;

    public void setOnReportEventListener(IPlayer.OnReportEventListener listener) {
        this.mOnReportEventListener = listener;
    }

    private void onReportEvent(Map<String, String> map) {
        if (mOnReportEventListener != null) {
            mOnReportEventListener.onEventParam(map);
        }
    }
    ...
    public PlayerQosInfo getQosInfo() {
        ...
        if (aliPlayer != null) {
            qosInfo.vdps = 0;
            qosInfo.vfps = (Float)aliPlayer.getOption(IPlayer.Option.RenderFPS);

            qosInfo.width = aliPlayer.getVideoWidth();
            qosInfo.height = aliPlayer.getVideoHeight();

            float audioBitrate = (float)aliPlayer.getOption(IPlayer.Option.AudioBitrate);
            float videoBitrate = (float)aliPlayer.getOption(IPlayer.Option.VideoBitrate);
            qosInfo.bitrate = (long)(audioBitrate + videoBitrate);
            qosInfo.tcpSpeed = qosInfo.bitrate;

            qosInfo.startPlayTime = 0L;
            qosInfo.firstScreenTime = 0L;
            qosInfo.endPlayTime = 0L;
